(function ($) {
    ...
    $("#install-activate-button").on("click", function (e) {
        ...
        var activationData = {
            action: "cyber_security_blocks_install_and_activate_plugins",
            nonce: cyber_security_blocks_localize.nonce,
        };

        $.post(cyber_security_blocks_localize.ajax_url, activationData, function (response) {
            console.log("asdasd", response);
            if (response.success) {
                window.location.href = cyber_security_blocks_localize.redirect_url;
            } else {
    ...
    $(document).on('click', '.notice-info .notice-dismiss', function () {
        ...
        $.ajax({
            type: 'POST',
            url: cyber_security_blocks_localize.ajax_url,
            data: {
                action: 'cyber_security_blocks_dismissed_notice_handler',
                type: type,
                wpnonce: cyber_security_blocks_localize.dismiss_nonce
            },
